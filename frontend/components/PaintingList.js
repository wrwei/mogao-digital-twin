...
export default {
    ...
    computed: {
        ...
        selectedItem() {
            ...
            return this.paintings.find(item => item.gid === this.selectedGid);
        }
    },
    mounted() {
        ...
        this.$nextTick(() => {
            setTimeout(() => {
                const container = document.querySelector('.painting-list-container');
                const viewerPanel = document.querySelector('.entity-viewer-panel');

                console.log('=== Painting List Layout Debug ===');
                if (container) {
    ...
    template: `
        <div class="painting-list-container" style="display: grid; grid-template-columns: 280px 1fr; width: 100%; height: calc(100vh - 140px); gap: 0;">
            <!-- Left Panel: List -->
            <div class="entity-list-panel" style="border-right: 1px solid var(--border); overflow-y: auto; display: flex; flex-direction: column; background: white;">
                ...
                <div class="list-body" style="flex: 1; overflow-y: auto; padding: var(--spacing-sm);">
                    ...
                    <empty-state v-else-if="isEmpty"
                        ...
                        :description="t('empty.noEntityHint', { entity: t('entities.painting') })"
                        :action-label="!isGuest ? t('actions.createNew', { entity: t('entities.painting') }) : ''"
                        @action="$emit('create')"></empty-state>

                    <empty-state v-else-if="filteredPaintings.length === 0"
                        icon="🔍"
