/**
 * Mural Form Component
 * Auto-generated from mogao_dt.ecore
 * Create/Edit form for 壁画
 */
...
export default {
    ...
    data() {
        return {
            form: {
                name: this.mural?.name || '',                description: this.mural?.description || '',                label: this.mural?.label || '',                creationPeriod: this.mural?.creationPeriod || '',                lastInspectionDate: this.mural?.lastInspectionDate || null,                inspectionNotes: this.mural?.inspectionNotes || '',                material: this.mural?.material || '',                period: this.mural?.period || '',                conservationStatus: this.mural?.conservationStatus || '',                width: this.mural?.width || null,                height: this.mural?.height || null,                technique: this.mural?.technique || '',                reference: this.mural?.reference || {
                    gid: '',
                    modelLocation: '',
                    metadataLocation: '',
                    textureLocation: ''
                },
                environmentConditions: this.mural?.environmentConditions || {
                    gid: '',
                    name: '',
                    description: '',
                    timestamp: null
                },
                defects: this.mural?.defects || {
                    gid: '',
                    name: '',
                    description: '',
                    defectType: '',
    ...
    methods: {
        loadFormData(data) {
            ...
            this.form.technique = data.technique || '';
            if (data.reference) {
        ...
        async handleSubmit() {
            ...
            try {
                ...
                if (this.mode === 'create') {
                    const response = await api.murals.create(data);
                    this.$emit('created', response.data);
                } else {
                    const gid = this.mural.gid;
                    await api.murals.update(gid, data);
                    this.$emit('updated', { ...this.mural, ...data });
                }
                this.resetForm();
            } catch (error) {
                console.error('Form submission error:', error);
                this.$emit('error', error.response?.data?.message || error.message || this.t('actions.saveError', { entity: this.t('entities.mural') }));
            } finally {
                this.loading = false;
            }
        },
    ...
    template: `
        <form @submit.prevent="handleSubmit" class="form mural-form">
            <h2>{{ mode === 'create' ? t('common.create') : t('common.edit') }} {{ t('entities.mural') }}</h2>
            ...
            <div class="form-group" v-if="mode === 'edit' || true">
                <label class="form-label" for="technique">
                    {{ t('fields.technique') }}
                </label>
                ...
                <input
                    ...
                    @blur="markTouched('technique')"
                    class="form-input"
                    :class="{ 'form-input-error': errors.technique && touched.technique }"
                    :placeholder="t('fields.technique')"
                />

                <span v-if="errors.technique && touched.technique" class="form-error">
                    {{ errors.technique }}
                </span>
            </div>
