...
export default {
    ...
    data() {
        return {
            form: {
                name: this.statue?.name || '',                description: this.statue?.description || '',                label: this.statue?.label || '',                creationPeriod: this.statue?.creationPeriod || '',                lastInspectionDate: this.statue?.lastInspectionDate || null,                inspectionNotes: this.statue?.inspectionNotes || '',                material: this.statue?.material || '',                period: this.statue?.period || '',                conservationStatus: this.statue?.conservationStatus || '',                width: this.statue?.width || null,                depth: this.statue?.depth || null,                height: this.statue?.height || null,                subject: this.statue?.subject || '',                reference: this.statue?.reference || {
                    gid: '',
                    modelLocation: '',
                    metadataLocation: '',
                    textureLocation: ''
                },
                environmentConditions: this.statue?.environmentConditions || {
                    gid: '',
                    name: '',
                    description: '',
                    timestamp: null
                },
                defects: this.statue?.defects || {
                    gid: '',
                    name: '',
                    description: '',
                    defectType: '',
    ...
    methods: {
        ...
        async handleSubmit() {
            ...
            try {
                ...
                // Clean empty strings and _id to avoid validation errors
                ...
                if (this.mode === 'create') {
                    const response = await api.statues.create(data);
                    this.$emit('created', response.data);
                } else {
                    const gid = this.statue.gid;
                    await api.statues.update(gid, data);
                    this.$emit('updated', { ...this.statue, ...data });
                }
                this.resetForm();
            } catch (error) {
                console.error('Form submission error:', error);
                this.$emit('error', error.response?.data?.message || error.message || this.t('actions.saveError', { entity: this.t('entities.statue') }));
            } finally {
                this.loading = false;
            }
        },
    ...
    template: `
        <form @submit.prevent="handleSubmit" class="form statue-form">
            <h2>{{ mode === 'create' ? t('common.create') : t('common.edit') }} {{ t('entities.statue') }}</h2>
